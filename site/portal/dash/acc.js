                ...
                try {
                    ...
                    if (!isEmpty(data)) {
                        ...
                        if (!isJsonArray(jdata)) {
                            ...
                            document.getElementById('w_userid').value = value.student_id;
                            document.getElementById('w_name').value = value.name;
                            document.getElementById('w_no').value = value.phone;


                            document.getElementById('d_userid').value = value.student_id;
                            document.getElementById('d_name').value = value.name;
                            document.getElementById('d_no').value = value.phone;


                            document.getElementById('sw_userid').value = value.student_id;
                            document.getElementById('sw_name').value = value.name;
                            document.getElementById('sw_no').value = value.phone;


                            document.getElementById('sd_userid').value = value.student_id;
                            document.getElementById('sd_name').value = value.name;
                            document.getElementById('sd_no').value = value.phone;


                            document.getElementById('r_userid').value = value.student_id;
                            document.getElementById('r_name').value = value.name;


                            document.getElementById('s_userid').value = value.student_id;
                            document.getElementById('s_name').value = value.name;
                            document.getElementById('v_s_userid').value = value.student_id;
                            ...
                            getbalanceTable(value.student_id, 'accounts_table', 'sacco_actions');

                            loadtransaction(value.student_id);
                            loadpayments(value.student_id, '2', 'pay_table', 'pay_body');
                            loadpayments(value.student_id, '1', 'depo_table', 'depo_body');
                            loadpayments(value.student_id, '0', 'with_table', 'with_body');
                            checkClubSacco(value.student_id);
                            //getSacco_role(value.student_id, 'NA');
                            //loadSchool();
                ...
                try {
                    ...
                    if (!isEmpty(data)) {
                        ...
                        if (!isJsonArray(value)) {
                            ...
                            checkApproval_Status(value.club_id, 'null', input, value.status, 'r_table', 'r_body_table', 'btn_request_sacco');
                            //loadRequest(value.club_id, 'payid', input, 'r_table', 'r_body_table');
                            loadRequest_status(value.club_id, 'rs_table', 'rs_body_table');
                        ...
                        if (value.status === '2') {
                            document.getElementById('btn_withdraw_sacco').style.display = 'block';
                            document.getElementById('btn_withdraw_approval').style.display = 'block';
                        } else if (value.status === '1') {
                            //document.getElementById('btn_request_sacco').style.display = 'block';
                        }
                    } else {
...
function withdraw() {
    ...
    if (parseInt(removeCommas(bal)) >= parseInt(removeCommas(amount))) {
        ...
        if (valPhone_x(tel, "Provid Correct Phone Number") === false || valForm(amount, "Provid Correct Amount") === false) {
            //empty fields
        } else {
            addtransaction(userid, accid, tel, amount, "Withraw", "1");
            minusAcc(accid, userid, amount);
            addPayWith(accid, userid, 'Withraw', '2', tel, 'User Withraw', amount, '0');
            with_yo(tel, amount, userid + "Withraw " + amount + "from Acc:" + accid);
        }
    }
}
...
function deposit_credit() {
    ...
    if (valPhone_x(tel, "Provid Correct Phone Number") === false || valForm(amt, "Provid Correct Amount") === false) {
        //empty fields
    } else {
        addtransaction(userid, accid, tel, amt, "Deposit", "2");
        addAcc(accid, userid, amt);
        addPayWith(accid, userid, 'Deposit', '1', tel, 'User Deposit', amt, '0');
        depo_yo(tel, amt, userid + "Deposits " + amt + "to Acc:" + accid);
    }
...
document.getElementById('btn_withdraw_sacco').addEventListener('click', withSacco_Div);
function withSacco_Div() {
    var userid = document.getElementById('sw_userid').value;
    //console.log(userid);
    clubAccountID(userid, 'sw_sel_acc');
    //getbalance(userid, 'w_bal');
    //getItem(item_id);
    valTel_x('sw_no');
...
document.getElementById('btn_request_sacco').addEventListener('click', withSacco_Requests);
function withSacco_Requests() {
    ...
    //getItem(item_id);
    //valTel_x('sd_no');
    $('#withdraw_request').modal('show');
}




document.getElementById('btn_withdraw_approval').addEventListener('click', withSacco_Requests_status);
function withSacco_Requests_status() {
    //var userid = document.getElementById('r_userid').value;
    //console.log(userid);
    //clubAccountID(userid, 'sw_sel_acc');
    //getbalance(userid, 'w_bal');
    //getItem(item_id);
    //valTel_x('sd_no');
    $('#withdraw_request_status').modal('show');
}
...
function deposit_sacco() {
    let userid = document.getElementById("sd_userid").value;
    ...
    if (valPhone_x(tel, "Provid Correct Phone Number") === false || valForm(amount, "Provid Correct Amount") === false) {
        ...
        addPayWith(accid, userid, 'Deposit', '1', tel, 'Sacco', amount, '0');
        addAcc(accid, userid, amount);
        depo_yo(tel, amount, userid + "Deposits " + amount + "to Acc:" + accid);
        addtransaction(userid, accid, tel, amount, "Deposit", "2");
    }
...
document.getElementById('sw_btn_with').addEventListener('click', withDrawSacco);
function withDrawSacco() {
    let userid = document.getElementById("sw_userid").value;
    let tel = document.getElementById("sw_no").value;
    let amount = document.getElementById("sw_amt").value;
    let reason = document.getElementById("sw_reason").value;
    let accid = $("#sw_sel_acc :selected").attr('id');
    //let acc_nam = $("#sw_sel_acc :selected").attr('name');
    if (valPhone_x(tel, "Provid Correct Phone Number") === false || valForm(amount, "Provid Correct Amount") === false) {
        //empty fields
    } else {
        addPayWith(accid, accid, reason, '2', tel, 'Sacco', amount, '1');
        //addtransaction(userid, accid, tel, amount, "Deposit", "2");
        //addAcc(accid, userid, amount);
    }
